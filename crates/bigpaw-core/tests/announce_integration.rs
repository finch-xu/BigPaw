...
fn poke_wakes_peer_via_unicast_announcement() {
    ...
    let a = AnnounceService::start(&ida, "alice", 24917, DEFAULT_ANNOUNCE_PORT, txa).unwrap();
    let _b = AnnounceService::start(&idb, "bob", 24918, DEFAULT_ANNOUNCE_PORT, txb).unwrap();
